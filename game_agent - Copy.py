...
class MinimaxPlayer(IsolationPlayer):
    ...
    def minimax(self, game, depth):
        ...
        **********************************************************************
        ...
        #default assuming we are maximizing
        currentPlayer = self
        if self is not game._active_player:
            # if we are not the active player, the opponent is minimizing
            currentPlayer = game.get_opponent(self)
        potentialMoves = game.get_legal_moves(currentPlayer)

        if len(potentialMoves) == 0 or depth == 0:
            ...
            for move in potentialMoves:
                if depth == 1:
                    score,child_move = (self.score(game.forecast_move(move),self),move)
                else:
                    score,child_move = (self.score(game.forecast_move(move),game.get_opponent(currentPlayer)),
                                        self.minimax(game.forecast_move(move), depth-1))
                scores_and_moves.add((score,move))
            if currentPlayer is self:
                bestMoveSet = max(scores_and_moves)
            else:
                bestMoveSet = min(scores_and_moves)
            bestMove = bestMoveSet[1]
        return bestMove
...
class AlphaBetaPlayer(IsolationPlayer):
    ...
    def alphabeta(self, game, depth, alpha=float("-inf"), beta=float("inf")):
        ...
        **********************************************************************
        ...
        scores_and_moves = set()
        currentPlayer = self
        if game._player_1 is not game._active_player:
            # if we are not the active player, the opponent is minimizing
            currentPlayer = game.get_opponent(self)
        potentialMoves = game.get_legal_moves(currentPlayer)

        if len(potentialMoves) == 0 or depth == 0:
            self.score(game, currentPlayer)
            bestMove = (-1,-1)
        ...
        for move in potentialMoves:
            if depth == 1:
                ...
                score,child_move = (self.score(game.forecast_move(move),self),
                                   self.alphabeta(game.forecast_move(move), depth-1, alpha, beta))
            if currentPlayer is game._active_player:
                if score > beta:
            ...
            if currentPlayer is game._player_1:
                bestMoveSet = max(scores_and_moves)
            else:
                bestMoveSet = min(scores_and_moves)
